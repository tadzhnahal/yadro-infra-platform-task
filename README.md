...
Во время проверки `https://httpstat.us` может рвать соединение и не возвращать HTTP-ответ. Поэтому скрипт поддерживает переменную окружения `HTTP_STATUS_BASE_URL`, через неё можно вручную передать другой адрес без правки кода. Для проверки я использовал совместимый сервис `https://tools-httpstatus.pickup-services.com`. Он принимает те же пути со статус-кодами: `/200`, `/404`, `/500` и другие. При этом по умолчанию в коде остаётся значение из задания: `https://httpstat.us`.
...
По умолчанию Ansible playbook использует адрес из задания: `https://httpstat.us`. Если нужно проверить сценарий на совместимом сервисе, можно передать другой базовый адрес через переменную окружения `HTTP_STATUS_BASE_URL`.

Я разделил Ansible-задачи на несколько файлов в `ansible/tasks`, потому что общий playbook стал слишком большим. Главный `playbook.yml` содержит общий порядок работы, а task-файлы хранят команды по подготовке Docker, сборке образа, запуску контейнера и проверке логов.
...
```bash
HTTP_STATUS_BASE_URL=https://tools-httpstatus.pickup-services.com ansible-playbook playbook.yml --ask-become-pass
```
...
В логах появятся все пять запросов. При запуске с адресом из задания сервис может вернуть сетевые ошибки. При запуске с совместимым эндпоинтом для `200` и `302` будут успешные ответы, а для `404` и `500` будут ошибки из-за плохого HTTP-ответа.
